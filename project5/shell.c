...
void parseBuffer(char buffer[]){
	...
	if(size <= 3){
		...
		}else if(checkCommand("show processes",buffer, index)){
			interrupt(0x21, 0x0A, 0,0, 0);
		}else if(checkCommand("kill",buffer, index)){
			index = 4;
			if(buffer[index] == ' '){
				retrieveParameter(buffer, parameterName,index,6);
				interrupt(0x21, 0x0B, parameterName,0, 0);
			}else{
				printScreen("\r\nShell> Invalid kill call - Missing parameter\0");
			}
		}
